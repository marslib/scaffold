...
class Redis
{
     ...
     */
    public static function GET($key, $clusterId = 'default', $enableMethodCache = FALSE)
    {
        if ($enableMethodCache && isset(self::$CACHE['GET'][$key])) {
            $value = self::$CACHE['GET'][$key];
            log_message('Cache_Redis GET from method cache. key:' . $key . ' value:' . json_encode($value), LOG_DEBUG, 2);
            return $value;
        }
        $redis = RedisClient::getInstance($clusterId);
        $value = $redis->get($key);
        if ($enableMethodCache) {
            self::$CACHE['GET'][$key] = $value;
        }
        log_message('Cache_Redis GET. key:' . $key . ' value:' . $value, LOG_DEBUG, 2);
        return $value;
    }
    ...
    public static function SET($key, $value, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis SET. key:' . $key . ' value:' . $value . ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function SETEX($key, $value, $cacheTime, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis SETEX. key:' . $key . ' value:' . $value . ' ttl:' . $cacheTime . ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function EXPIRE($key, $cacheTime, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis EXPIRE. key:' . $key . ' ttl:' . $cacheTime . ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function DEL($key, $clusterId = 'default')
    {
        $redis = RedisClient::getInstance($clusterId);
        if (is_array($key)) {
            $result = call_user_func_array([ $redis, 'delete' ], $key);
            $key = implode(',', $key);
        } else {
            $result = $redis->delete($key);
        }
        log_message('Cache_Redis DEL. key:' . $key . ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function EXISTS($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis EXISTS. key:' . $key . ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function INCR($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis INCR. key:'. $key. ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function INCRBY($key, $increment, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis INCRBY. key:'. $key. ' decrement:' .$increment .' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function DECR($key, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis DECR. key:'. $key. ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function DECRBY($key, $decrement, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis DECRBY. key:'. $key. ' decrement:' .$decrement .' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function HGETALL($key, $clusterId = 'default', $enableMethodCache = TRUE)
    {
        if ($enableMethodCache && isset(self::$CACHE['HGETALL'][$key])) {
            ...
            log_message('Cache_Redis HGETALL from method cache. key:' . $key . ' result:' . json_encode($result), LOG_DEBUG, 2);
            return $result;
        }
        $redis = RedisClient::getInstance($clusterId);
        ...
        log_message('Cache_Redis HGETALL. key:' . $key . ' result:' . json_encode($result), LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function HEXISTS($key, $field, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis HEXISTS. key:' . $key . ' field:' . $field . ' result:' . $result, LOG_DEBUG, 2);
        return $result;
    }
    ...
    public static function HMSET($key, $hashTable, $clusterId = 'default')
    {
        ...
        log_message('Cache_Redis HMSET. key:' . $key . ' hashtable:' . json_encode($hashTable) . ' result:' . json_encode($result), LOG_DEBUG, 2);
        return $result;
    }

    /**
     * @param $key string
     * @param $kvs array  [ score1 =>  value1, ... ]
     * @param $clusterId string
     * @return int
     */
    public static function ZADD($key, $kvs, $clusterId = 'default')
    {
        $redis = RedisClient::getInstance($clusterId);
        $result = 0;
        foreach ($kvs as $score => $value) {
    ...
    /**
     * @param $key string
     * @param $min int|string
     * @param $max int|string
     * @param $clusterId string
     * @param $options array  array('withscores' => TRUE, 'limit' => array(1, 1)
     * @return array
     */
    public static function ZRANGEBYSCORE($key, $min, $max, $clusterId = 'default', $options = [])
    {
        $redis = RedisClient::getInstance($clusterId);
     ...
     * @return int
     */
    public static function ZREMRANGEBYSCORE($key, $min, $max, $clusterId = 'default')
    {
        $redis = RedisClient::getInstance($clusterId);
    ...
    /**
     * @param $key string
     * @param $members string|int|array
     * @param $clusterId string
     * @return int
     */
    public static function ZREM($key, $members, $clusterId = 'default')
    {
     ...
     * 根据member 返回score
     * @param $key string
     * @param $member string|int|array
     * @param $clusterId string
     * @return int
     */
    public static function ZSCORE($key, $member, $clusterId = 'default')
    {
        $redis = RedisClient::getInstance($clusterId);
