...
function param_post($def, $prefix=null, &$scope=null, $default=array()) {
  ...
  if (empty($source) && isset($_SERVER['CONTENT_TYPE']) && preg_match('@application/json@i', $_SERVER['CONTENT_TYPE'])) {
  ...
  return _param_handle($source, $def, $prefix, $scope, $default);
}
    ...
    if ( ! is_numeric($flags)) {
        ...
        foreach (explode('|', $flags) as $paramType) {
          ...
          if ( ! isset($GLOBALS[$paramType])) {
            _param_error('PARAM: invalid param type:' . $paramType);
            continue;
          }
